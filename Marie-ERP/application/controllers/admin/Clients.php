...
class Clients extends AdminController
{
    ...
    public function client($id = '')
    {
        ...
        $group         = !$this->input->get('group') ? 'profile' : $this->input->get('group');//indha idathula ne default ha edhu open aganum nu kudukanum
        $data['group'] = $group;
        ...
        // Customer groups
        $data['groups'] = $this->clients_model->get_groups();//groups dropdown data

        if ($id == '') {//create form
            $title = _l('add_new', _l('client_lowercase'));            
        } else {
            //edit form this is menus page
            $client                = $this->clients_model->get($id);//client data
            $data['customer_tabs'] = get_customer_profile_tabs();
//tabs namba ready pananum
            ...
            // Fetch data based on group
            if ($group == 'profile') {
                $data['customer_groups'] = $this->clients_model->get_customer_groups($id);//group dropdown
